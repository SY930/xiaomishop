...
<script>
  ...
  import {getCartList,delCarts,updateCart,editCheckAlls} from '../api'
  export default{
      data(){
          return {
              cartList:[],
    ...
    methods:{
      ...
      delCart(){
          delCarts(this.delItem.productId).then((res)=>{
              if(res.status=='0'){
      ...
      editCart(flag,item){
        ...
        if(flag=='add'){
          ...
          updateCart(item.productId,item.productNum,item.checked).then((res)=>{
              if(res.status=='0'){
                  let num =0;
                if(flag=='add'){
                  num = 1;
      ...
      toggleCheckAll(){
        //this.checkAllFlag 是通过computed实时计算出来的 发现长度相同又会赋值为true，所以需要用一个flag来记录当前的状态
          var flag = !this.checkAllFlag;
          this.cartList.forEach((item)=>{
              item.checked = flag?"1":"0"
          });
        editCheckAlls(flag).then((res)=>{
            if(res.status=='0'){
